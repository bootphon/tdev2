...
def test_get_transcription(disc, gold): 
    ...
    disc = Disc(pairs_path, gold)
    token_ngram, ngram = disc.get_transcription(
            's2301b', 232.849, 233.929,
            gold.phones)

    gold_ngram = ('n','aa','t','k','l','ae','m','ah','r','ih','ng','f','ow')

    assert (ngram == gold_ngram), 'Wrong ngram found, shoud have found\n {}\n but found\n {}\n'.format(gold_ngram, ngram)
...
def test_30ms(disc, gold):
     ...
     assert ngram_notbeg == ('l', 'uh', 'k', 'ow'), 'should not have found first phone because took less than 30ms of it'
     assert ngram_notend == ('ay', 'l', 'uh','k') , 'should not have found last phone because took less than 30ms of it'
     assert ngram_good == ('ay', 'l', 'uh', 'k', 'ow'), 'should have found last phone because took more than 30ms of it'
...
def test_50percent(disc, gold):
     ...
     assert ngram_notbeg == ('hh', 'ah'), 'should not have found first phone because took less than 50% of it'
     assert ngram_notend == ('ah',) , 'should not have found last phone because took less than 50% of it'
     assert ngram_good == ('ah', 'n'), 'should have found last phone because took more thant 50% of it'
