from WDE.measures.grouping import Grouping


def test_same_pairs_and_class(gold, disc_clusters, disc_pairs):
    """ results should be the same if given in pairs or in clusters"""
    group_clusters = Grouping(gold, disc_clusters)
    group_pairs = Grouping(gold, disc_pairs)
...
def test_weights(gold, disc_clusters):
    good_pairs = [
    ...
    group = Grouping(gold, disc_clusters)
...
def get_found_pairs(gold, disc_clusters):
    pass


def get_gold_pairs(gold, disc_clusters):
    pass
