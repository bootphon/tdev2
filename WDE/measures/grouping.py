...
class Grouping(Measure):
    ...
    def get_gold_pairs(self):
        ...
        self.gold_pairs = {tuple(sorted((f1, f2),
    ...
    def get_found_pairs(self):
        ...
        for class_nb in self.clusters:
            self.found_pairs = self.found_pairs.union(set(combinations(
                                self.clusters[class_nb],2)))
            self.found_types = self.found_types.union({ngram 
                       for _,_,_,token_ngram,ngram in self.clusters[class_nb]})
        # order found pairs
        self.found_pairs = {tuple(sorted((f1, f2), 
                                  key = lambda f: (f[0], f[1])))
